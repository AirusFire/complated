...
module.exports = {
  name: "config",
  description: "Bot ayarlarını düzenleyebilirsiniz.",
  usage: "",
  permissions: {
...
`);
    ...
    if (em._emoji.name === "1️⃣") {
      await client.sendTime(
        message.channel,
        "Prefixi ne olarak değiştirmek istiyorsunuz?"
      );
      ...
      client.sendTime(
        message.channel,
        `Prefix başarıyla kaydedildi! Yeni prefix: \`${prefix}\``
      );
    } else {
      await client.sendTime(
        message.channel,
        "Lütfen DJ rolünü belirtin."
      );
      ...
      if (!role.mentions.roles.first())
        return client.sendTime(
          message.channel,
          "Lütfen sadece DJ rolünü belirtin."
        );
      ...
      client.sendTime(
        message.channel,
        "DJ rolü başarıyla kaydedildi! Yeni DJ rolü: <@&" + role.id + ">"
      );
    }
  },
